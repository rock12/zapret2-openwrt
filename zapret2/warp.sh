...
EOF
        ...
        # Generate self-contained WARP.conf with AmneziaWG obfuscation (bypasses Russian TSPU blocks)
        cat > "$WARP_DIR/WARP.conf" <<-EOF
[Interface]
PrivateKey = $priv
Address = $v4, ${v6:-2606:4700:110:801f:ef38:34a9:84bc:a5a0}
DNS = 1.1.1.1, 1.0.0.1
MTU = 1280
S1 = 0
S2 = 0
...
EOF
        chmod 600 "$WARP_DIR/WARP.conf"
        _log "Устройство успешно зарегистрировано автоматически с AmneziaWG! IP: $v4"
        return 0
    fi
...
EOF
    chmod 600 "$WARP_DIR/WARP.conf"
    _log "Устройство успешно зарегистрировано с AmneziaWG! IP: $v4"
    return 0
}
...
warp_uci_setup() {
    ...
    local awg_conf=""
    if [ -r "$WARP_DIR/WARP.conf" ]; then
        awg_conf="$WARP_DIR/WARP.conf"
    elif [ -r /root/WARP.conf ]; then
        awg_conf="/root/WARP.conf"
    fi

    local has_awg=0
    if [ -x /usr/bin/awg ] && [ -n "$awg_conf" ]; then
        has_awg=1
    fi
